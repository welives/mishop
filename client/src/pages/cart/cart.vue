<template>
  <view style="background-color: #f5f5f5;">
    ...
    <view class="bg-white px-2">
      <block v-for="(item, index) in dataList" :key="index">
        <view class="flex align-center py-2 border-bottom border-light-secondary" style="height: 230rpx;">
          <label class="flex align-center justify-center flex-shrink-0" style="width: 80rpx; height: 80rpx;">
            <radio color="#fd6801" :value="item.id.toString()" :checked="checkedAll" />
          </label>
          <image
    ...
    >
      <label class="flex align-center justify-center flex-shrink-0" style="width: 100rpx;">
        <radio value="1" color="#fd6801" />
      </label>
      <view class="flex-fill flex align-center justify-center">
        <text class="font-md">合计：</text>
        <price sales="9999" salesUnitFont="font" />
      </view>
